...
# Intentamos la importación alternativa para evitar errores de submódulos
try:
...
class EstadoPunto:
    """
    Gestiona las coordenadas lógicas del punto sin necesidad de inicializar Pygame.
    """
    def __init__(self):
        self.mapa = Mapa() # Instancia del mapa de Matplotlib
        
        # Posición inicial lógica del punto en la pantalla virtual
        self.pos_x = ANCHO // 2
        self.pos_y = ALTO // 2
    ...
    def desplazar(self, delta_x, delta_y):
        # Mantiene el factor de escala suave que tenías anteriormente
        self.pos_x += delta_x * 0.5
        self.pos_y += delta_y * 0.5
        
        # Mantener el punto dentro de los límites virtuales
            ...
            if Mano_cerrada(resultados):
                ...
                if not mano_agarre:
                    ...
                    if time.time() - inicio_agarre > 0.01:
                        ...
                        # Movemos las coordenadas internas
                        estado_punto.desplazar(dx, dy)

                        # Conversión para el mapa mundi (-180 a 180 en X, 90 a -90 en Y)
                        x_mapa = (estado_punto.pos_x / ANCHO * 360) - 180
                        y_mapa = 90 - (estado_punto.pos_y / ALTO * 180)
                        
                        # Actualizamos el marcador dinámico de Matplotlib
